<template>
  <section class="bg-blue-50">
    <div
      class="max-w-300 mx-auto px-4 md:px-10 py-10 md:py-16 flex flex-col lg:flex-row items-center gap-8 lg:gap-10"
    >
      <!-- Text -->
      <div class="w-full lg:flex-1 lg:max-w-xl">
        <h1
          class="text-3xl md:text-4xl font-extrabold text-brand-blue leading-tight mb-4 whitespace-pre-line"
        >
        ...
        <div class="flex gap-2 md:gap-2.5 mt-2.5">
          ...
          >
            <span class="text-xl md:text-2xl font-extrabold block">{{
              unit.value
            }}</span>
...
function update(): void {
  ...
  diff -= hv * 3600000;
  const mv = Math.floor(diff / 60000);
  diff -= mv * 60000;
